...
metadata = metadata[!is.na(metadata$infection_status_updated),]
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("#OTU ID"), 
                                        metadata = metadata, group = "infection_status_updated", daa_method = "DESeq2")
...
#this line will change for each dataset. 106 represents the number of samples in the filtered abundance table
abundance_desc = abundance_desc[,-c(106:ncol(abundance_desc))] 

# Generate a heatmap
pathway_heatmap(abundance = abundance_desc %>% column_to_rownames("feature"), metadata = metadata, group = "infection_status_updated")

# Generate pathway PCA plot
pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("pathway"), metadata = metadata, group = "infection_status_updated")
...
res =  DEseq2_function(abundance_data_filtered, metadata, "infection_status_updated")
